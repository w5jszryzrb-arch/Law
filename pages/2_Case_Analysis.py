...
with tab1:
    if not case_docs:
        st.warning("No case law documents uploaded. Upload cases in the Documents page.")
    else:
...
with tab3:
    if len(case_docs) < 2:
        st.warning("Upload at least 2 cases to use comparison mode.")
    else:
...
with tab4:
    if not case_docs:
        ...
        if "case_chat_case_id" not in st.session_state:
            st.session_state["case_chat_case_id"] = None

        if chat_case and st.session_state["case_chat_case_id"] != chat_case["doc_id"]:
            st.session_state["case_chat_history"] = []
